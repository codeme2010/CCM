            ...
            default:
                queryBuilder.setTables(
                        "(select t1._id,case(strftime('%d','now','localtime')-t1.huankuanri>0)\n" +
                                "when 1 then STRFTIME ('%m-%d',date('now','start of month','+1 month','+'|| t1.huankuanri ||' day','-1 day','localtime')) \n" +
                                "else STRFTIME ('%m-%d',date('now','start of month','+'|| t1.huankuanri ||' day','-1 day','localtime')) end as huankuanriqi,t1.kadaihao, case when t2.huankuane isnull then '无需还款' else round(t2.huankuane,2) end as huankuane, case when t3.yue is null then t3.zonge else round(t3.yue,2) end as yue,t4.mianxiqi\n" +
                                ...
                                "WHEN '110' THEN DATE ('now', 'start of month', '+' || [zhangdanri] || ' day', '-1 day','localtime') \n" +
                                "WHEN '000' THEN DATE ('now', 'start of month', '+' || [zhangdanri] || ' day', '-1 day', '-1 month','localtime') \n" +
                                "WHEN '011' THEN DATE ('now', 'start of month', '+' || [zhangdanri] || ' day', '-1 day','localtime') ELSE DATE ('now','localtime') END\n" +
                                ...
                                "when '011' then julianday(date('now', 'start of month', '+' || [huankuanri] || ' day', '-1 day', '+2 month','localtime'))-julianday(date('now','localtime'))\n" +
                                "else julianday(date('now', 'start of month', '+' || [huankuanri] || ' day', '-1 day', '+1 month','localtime'))-julianday(date('now','localtime')) end as mianxiqi\n" +
